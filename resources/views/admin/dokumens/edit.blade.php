                        ...
                        <form action="{{ route('dokumens.update', $dokumen) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                              ...
                              <div>
                                    ...
                                    <textarea id="deskripsi" name="deskripsi" rows="4"
                                          ...
                                          class="w-full px-4 py-2.5 bg-slate-50  border border-slate-300  rounded-lg text-slate-900  placeholder-slate-500  focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none">{{ old('deskripsi', $dokumen->deskripsi) }}</textarea>
                                    @error('deskripsi')
                                    <p class="text-red-600  text-sm mt-1">{{ $message }}</p>
                                    @enderror
                              </div>
                              <div>
                                    ...
                                    <div class="relative border-2 border-dashed border-slate-300  rounded-lg p-6 transition-colors hover:border-blue-400  cursor-pointer group" id="uploadArea">
                                          ...
                                          <div id="placeholderContent" class="text-center">
                                                ...
                                                <p class="text-xs text-slate-500 ">PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX atau TXT (max 10MB)</p>
                                          </div>
                                    ...
                                    <div class="mt-4 p-4 bg-blue-50  rounded-lg border border-blue-200 ">
                                          <p class="text-sm text-blue-900 ">
                                                <strong>File Saat Ini:</strong> {{ basename($dokumen->file) }} ({{ number_format($dokumen->ukuran_file / 1024, 2) }} KB)
                                          </p>
                                    </div>
                                    @endif
