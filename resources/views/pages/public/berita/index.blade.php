      ...
      <div class="max-w-6xl mx-auto py-12 px-4">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
                  ...
                  <div class="lg:col-span-1 space-y-6">
                        <!-- Search Box -->
                        <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-md border border-slate-200 dark:border-zinc-800 p-5 sticky top-6">
                              <h3 class="text-base font-bold text-slate-900 dark:text-white mb-4 pb-3 border-b border-slate-200 dark:border-zinc-800">
                                    Cari Berita
                              </h3>
                              <form action="{{ route('berita.public.index') }}" method="GET" class="flex flex-col gap-3">
                                    <div class="relative">
                                          <input type="text"
                                                ...
                                                class="w-full px-3 py-2 bg-slate-50 dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 rounded-lg text-sm text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all">
                              ...
                              <div class="mt-3 pt-3 border-t border-slate-200 dark:border-zinc-800">
                                    @if (request('search'))
                                    <p class="text-xs text-slate-600 dark:text-slate-400 mb-2">Pencarian: <span class="font-semibold text-slate-900 dark:text-white">{{ request('search') }}</span></p>
                                    @endif
                                    <a href="{{ route('berita.public.index') }}" class="text-xs text-emerald-600 dark:text-emerald-400 hover:underline font-medium">
                                          Hapus Filter
                                    </a>
                              </div>
                              @endif
                        ...
                        <!-- Categories -->
                        <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-md border border-slate-200 dark:border-zinc-800 p-5 sticky top-64">
                              <h3 class="text-base font-bold text-slate-900 dark:text-white mb-4 pb-3 border-b border-slate-200 dark:border-zinc-800">
                                    Kategori
                              </h3>
                              ...
                              <div class="space-y-2">
                                    <a href="{{ route('berita.public.index') }}"
                                          class="flex items-center justify-between px-3 py-2 rounded-lg transition-colors text-sm {{ !request('category') ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 font-medium' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-zinc-800' }}">
                                          <span>Semua Berita</span>
                                          <span class="text-xs bg-slate-200 dark:bg-zinc-700 px-2 py-0.5 rounded-full">
                                    ...
                                    <a href="{{ route('berita.public.index', ['category' => $category->id]) }}"
                                          class="flex items-center justify-between px-3 py-2 rounded-lg transition-colors text-sm {{ request('category') == $category->id ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 font-medium' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-zinc-800' }}">
                                    ...
                                    @endforeach
                                    @else
                                    <p class="text-xs text-slate-500 dark:text-slate-400 py-2">Tidak ada kategori</p>
                                    @endif
                              </div>
                        </div>
                  </div>
                  ...
                  <div class="lg:col-span-3">
                        ...
                        <div class="text-center py-20">
                              ...
                              <p class="text-slate-600 dark:text-slate-400 font-medium">{{ __('Tidak ada berita yang sesuai dengan pencarian') }}</p>
                              <p class="text-slate-500 dark:text-slate-500 text-sm mt-1">{{ __('Coba ubah filter atau kata kunci pencarian') }}</p>
